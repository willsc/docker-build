...
For each host and filesystem, the script:
  1. Determines whether to perform a full backup (on the 1st day of the month or when no snapshot is available)
     or an incremental backup (using tar’s --listed-incremental option with a snapshot file).
  2. Copies (or creates) a local snapshot file to the remote host (in a temporary location) so that tar on the
     remote host can use it. For a full backup the snapshot file is removed so that tar will build a new snapshot.
...
def run_remote_tar(ssh_user, host, remote_snapshot, remote_path):
    ...
    """
    # Build the tar command. Note:
    #  - The snapshot file must be readable/writable by tar.
    #  - If the snapshot file does not exist on the remote host, tar will do a full backup.
...
def main():
    parser = argparse.ArgumentParser(description="Backup script using tar over SSH with gzipped archives and incremental backups (no rsync).")
    ...
    # Determine overall backup type: full if today is the 1st day of the month.
    overall_backup_type = "full" if today.day == 1 else "incremental"
    logging.info("Starting %s backup for %s", overall_backup_type, date_str)
    ...
    for host in config["hosts"]:
        ...
        for fs in host["filesystems"]:
            ...
            # For an incremental backup we need an existing snapshot file.
            ...
            if backup_mode == "full":
                # Remove any existing snapshot file so that tar will do a full backup.
                ...
                # Create an empty file to serve as a starting snapshot.
                with open(local_snapshot_file, "w") as f:
                    pass
            
            # Prepare remote temporary snapshot file path.
            # (Assumes that /tmp is writable on the remote host.)
            remote_snapshot = f"/tmp/backup_snapshot_{host_name}_{fs_name}.snar"
            
            # Copy the local snapshot file to the remote host.
            ...
            # Run the remote tar command.
            proc = run_remote_tar(ssh_user, host_name, remote_snapshot, remote_path)
            # Determine local archive directory and filename.
            if archive_enabled:
                ...
                # If archiving is disabled, use a temporary file (or you could stream elsewhere).
            ...
            try:
                with open(local_archive_file, "wb") as f_out:
                    # Stream the tarball from remote tar.
                    while True:
            ...
            # Copy the updated snapshot file back from the remote host.
            ...
            # Optionally remove the remote temporary snapshot file.
