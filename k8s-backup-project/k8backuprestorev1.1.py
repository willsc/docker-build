    ...
    for name, short in RESOURCE_TYPES:
        ...
        for item in resources.get('items', []):
            ...
            with open(file_path, 'w') as f:
                run(['kubectl'] + kubeconfig_arg + ['get', name, item_name, '-o', 'yaml'] + (['-n', namespace] if namespace else []), stdout=f)
...
def backup_helm(base_dir, namespace, kubeconfig):
    ...
    for rel in releases:
        ...
        with open(vals_file, 'w') as f:
            run(['helm', 'get', 'values', name, '--namespace', ns, '--all', '--output', 'yaml'] + kubeconfig_arg, stdout=f)
        with open(manifest_file, 'w') as f:
            run(['helm', 'get', 'manifest', name, '--namespace', ns] + kubeconfig_arg, stdout=f)
...
The script is now updated to back up each resource as an individual YAML file in the specified backup directory.
...
namespace-release-values.yaml

namespace-release-manifest.yaml
