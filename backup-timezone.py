...
def main():
    ...
    # The new directory structure will be: <archive_base>/<year>/<month>/<fs_name>/
    ...
    for host in config["hosts"]:
        ...
        # If a "timezone" key is provided in the host config, use it; otherwise, default to UTC.
        ...
        # Get the current time in the host's timezone.
        ...
        for fs in host["filesystems"]:
            ...
            try:
                ...
                if archive_enabled:
                    ...
                    # Build directory: <archive_base>/<year>/<month>/<fs_name>/
                ...
                try:
                    ...
                    retcode = proc.wait()
                    if retcode != 0:
                        logging.error("Host %s, filesystem %s: Remote backup command failed with code %s: %s",
                                      host_name, fs_name, retcode, stderr_output)
                        backup_success = False
                    else:
                        logging.info("Host %s, filesystem %s: Backup completed successfully.", host_name, fs_name)
                except Exception as e:
                    logging.error("Host %s, filesystem %s: Error saving backup: %s", host_name, fs_name, e)
                    backup_success = False
            except Exception as e:
                logging.error("Unexpected error processing backup for host %s, filesystem %s: %s", host_name, fs_name, e)
                backup_success = False
