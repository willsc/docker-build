...
def find_remote_gz_files(ssh_user, host, remote_dir, start_dt=None, end_dt=None):
    """
    Returns list of .gz files under remote_dir.
    If start_dt and end_dt provided, filters by modification time window.
    """
...
def main():
    parser = argparse.ArgumentParser(
        description="Copy .gz files from remote dirs (full or incremental) into dated local folders"
    )
    parser.add_argument("--config", type=str, default="backup_config.json",
                        help="Configuration JSON file path")
    group = parser.add_mutually_exclusive_group()
    group.add_argument("--full", action="store_true", help="Copy all .gz files (full backup)")
    group.add_argument("--incremental", action="store_true", help="Copy only today's .gz files (incremental backup)")
    ...
    if args.log_file:
        handlers.append(logging.FileHandler(args.log-file))
    ...
    # Default to incremental if neither specified
    mode = 'incremental' if not args.full else 'full'
    ...
    for host in cfg.get('hosts', []):
        ...
        for fs in host.get('filesystems', []):
            ...
            # Find .gz files based on mode
            if mode == 'full':
                gz_files = find_remote_gz_files(ssh_user, name, remote_dir)
            else:
                gz_files = find_remote_gz_files(ssh_user, name, remote_dir, start_dt, end_dt)

            if not gz_files:
                logging.info("No .gz files to backup for %s:%s (mode=%s)", name, remote_dir, mode)
