    ...
    with open(out_path) as f_in:
        for line in f_in:
            ...
            if re.match(r"\s*\"filesystems\"\s*:\s*\[", line):
                entries.append(line)
                # capture leading whitespace on this line, add three spaces for entries
...
I’ve bumped the collapse indent from two to three spaces under "filesystems", so each line now aligns one extra level in. Give it a try and let me know if it’s spot-on now!
