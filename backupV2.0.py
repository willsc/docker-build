...
This version is timezone aware in a minimal way using a manual mapping.
For example, if a host's configuration includes:
    "timezone": "Africa/Johannesburg"
the script will assume that local time for that host is UTC+2.
If no timezone is specified (or it is unrecognized), UTC is used.
...
**New Behavior:**
If the remote path ends with ".gz", it is assumed that the file is already a gzip
archive, so the script will simply use "cat" to stream the file without compressing it.
  ...
  --incremental    Force an incremental backup.
                   In incremental mode, the script determines a backup date as follows:
                     ...
                     • If the backup is started early (before the cutoff hour),
                       then it backs up files created/modified between yesterday’s midnight and today’s midnight.
...
def run_remote_backup(ssh_user, host, remote_path, backup_mode, threshold_date=None, upper_date=None):
    ...
    If the remote_path ends with ".gz", then the file is assumed to be a pre-compressed
    gzip archive and the command is simply "cat <remote_path>".
    
    For a full backup (if remote_path is not a .gz file), the command is:
         cd <remote_path> && tar -czf - .
         
    For an incremental backup (if remote_path is not a .gz file):
         cd <remote_path> && find . -type f -newermt 'threshold_date' [! -newermt 'upper_date'] | tar -czf - -T -
    """
    # If the remote path ends with .gz, copy the file as is.
    ...
    try:
        # Stream data in 4096-byte chunks.
...
def get_local_time(tz_str):
    """
    Get current local time for a given timezone string.
    Since neither zoneinfo nor pytz are available, we use a manual mapping.
    Currently supported:
      - "Africa/Johannesburg": UTC+2 (South Africa does not observe DST)
    ...
    offset_hours = mapping.get(tz_str, 0)
...
def main():
    ...
    # New directory structure: <archive_base>/<year>/<month>/<day>/<fs_name>/
    ...
    for host in config["hosts"]:
        ...
        # Determine the local time for this host using a manual mapping.
        ...
        for fs in host["filesystems"]:
            ...
            while attempt < max_attempts and not backup_success:
                ...
                try:
                    ...
                    if proc is None:
                        logging.error("Host %s, filesystem %s: Attempt %d: Failed to start remote backup command.",
