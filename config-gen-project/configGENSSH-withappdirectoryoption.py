...
{
  "services": { "SERVICE_NAME": ["log"], "OTHER_SERVICE": ["log", "jnl"] },
  "default": ["log", "jnl"]
}

Usage:
  python3 generate_backup_config.py  --hosts host1,host2  --user sysuser
    --timezone Europe/London  --archive-base-dir '/Delta1/Prod Backups'
    --remote-data-root '/local/1/home/sysuser/deploy/data'
    --output /path/to/config.json  [--service-config svc_config.json]
    [--ssh-retries 3] [--ssh-delay 5] [--extra-path /my/custom/path]
"""
import subprocess
import json
import argparse
...
def parse_up_services(sta_output):
    pattern = re.compile(r"^\s*(?P<name>[^:]+):")
    return [m.group('name').strip() for line in sta_output.splitlines() if (m := pattern.match(line))]
...
def write_and_collapse(config, out_path):
    ...
    # Collapse each filesystem block to one line
    entries = []
    fs_indent = None
...
def main():
    ...
    p.add_argument(
        ...
        action='append',
        metavar='PATH',
        help='Additional filesystem path to include verbatim (absolute or with wildcards)'
    )
    args = p.parse_args()
    ...
    for host in hosts:
        raw = run_sta(host, args.user, args.sta_cmd,
                      timeout=30, retries=args.ssh_retries, delay=args.ssh_delay)
        ...
        # Add any extra absolute/wildcard paths verbatim
        if args.extra_path:
            for extra in args.extra_path:
                name = os.path.basename(extra.rstrip('/'))
                filesystems.append({'name': name, 'path': extra})
    ...
    while attempt <= 2:
        ...
        if validate_config(out_path):
            ...
            print(f"Warning: Generated config invalid on attempt {attempt}. Retrying...", file=sys.stderr)
            attempt += 1
...
I’ve integrated the --extra-path (-e) option directly into the script. It now collects any additional paths (wildcards allowed) and appends them under each host’s filesystems. Let me know if you’d like any further tweaks!
