...
def run_remote_backup(ssh_user, host, remote_path, backup_mode, threshold_date=None, upper_date=None):
    ...
    if backup_mode == "full":
        ...
        if upper_date:
            # Backup only files modified between threshold_date (inclusive)
            # and upper_date (exclusive).
            cmd = (f"cd {remote_path} && find . -type f -newermt '{threshold_date}' "
                   f"! -newermt '{upper_date}' | tar -czf - -T -")
        else:
            # Backup files modified on or after threshold_date.
...
def main():
    ...
    # Process target hosts if specified.
    target_hosts = None
    if args.target_host:
    ...
    # Note: The archive_filename_format from the config is now overridden by the mode label logic.
    ...
    # Determine which date to use as threshold for incremental backup.
    # We use a cutoff hour (default 4 AM). If the current time is before 4 AM,
    # then assume the backup is running after midnight and should cover yesterday's files.
    ...
    # For incremental backups, if backup_date is less than today's date,
    # we will only include files modified between backup_date (inclusive) and today's date (exclusive).
    ...
    for host in config["hosts"]:
        ...
        for fs in host["filesystems"]:
            ...
            try:
                ...
                if archive_enabled:
                    archive_subdir = os.path.join(archive_base, host_name, fs_name, backup_date_str)
                    os.makedirs(archive_subdir, exist_ok=True)
                    # Include mode label in the filename.
                    mode_label = "full" if backup_mode == "full" else "incremental"
                    archive_filename = now.strftime(f"backup_{mode_label}_%Y-%m-%d_%H%M%S.tar.gz")
